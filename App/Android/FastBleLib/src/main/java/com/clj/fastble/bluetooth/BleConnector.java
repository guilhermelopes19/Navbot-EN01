...
public class BleConnector {
    ...
    public BleConnector withUUIDStringNotify(String serviceUUID, String characteristicUUID) {
        return withUUIDNotify(formUUID(serviceUUID), formUUID(characteristicUUID));
    }

    public BleConnector withUUIDNotify(UUID serviceUUID, UUID characteristicUUID) {
        if (serviceUUID != null && mBluetoothGatt != null) {
            mGattService = mBluetoothGatt.getService(serviceUUID);
        }
        if (mGattService != null && characteristicUUID != null) {
    ...
    public void enableCharacteristicNotify(BleNotifyCallback bleNotifyCallback, String uuid_notify,
                                           boolean userCharacteristicDescriptor, BluetoothGattCharacteristic characteristic) {
        if (characteristic != null
            ...
            setCharacteristicNotification(mBluetoothGatt, characteristic, userCharacteristicDescriptor, true, bleNotifyCallback);
        } else {
    ...
    private boolean setCharacteristicNotification(BluetoothGatt gatt,
        ...
        for (BluetoothGattDescriptor desc : descriptors) {
            if (desc.getCharacteristic().getUuid().equals(formUUID(UUID_CLIENT_CHARACTERISTIC_CONFIG_DESCRIPTOR))) {
                descriptor = desc;
                break;
            }
